...
final class Marketplace extends Component {
	...
	/**
	 * Checks if listing is recurring.
	 *
	 * @param  object $listing Listing object.
	 * @return bool
	 */
	public function is_recurring( $listing ) {
		return class_exists( 'wc_subscriptions' ) && $listing->get_subscription_period();
	}
	...
	public function update_vendor_balance( $vendor ) {
		...
		// Get payouts.
		$payouts = Models\Payout::query()->filter(
			[
		...
		foreach ( $orders as $order ) {
			$balance += $this->get_order_profit( $order );
		}

		foreach ( $payouts as $payout ) {
			$balance -= $payout->get_amount();
		}
	...
	protected function get_cart_fee( $cart, $per_vendor = false ) {
		...
		if ( hivepress()->get_version( 'bookings' ) ) {
			foreach ( $cart->get_fees() as $fee_args ) {
				if ( hivepress()->translator->get_string( 'security_deposit' ) === $fee_args->name ) {
					$total -= $fee_args->total;
	 ...
	 * @param bool     $normalize Normalize value?
	 * @return float
	 */
	public function get_order_total( $order, $normalize = true ) {
		...
		foreach ( $order->get_fees() as $fee ) {
			if ( in_array( $fee->get_name( 'edit' ), [ esc_html__( 'Service Fee', 'hivepress-marketplace' ), esc_html__( 'Direct Payment', 'hivepress-marketplace' ) ] ) ) {
				$total -= $fee->get_total( 'edit' );
			}
		}
		...
		if ( in_array( $new_status, [ 'processing', 'completed', 'refunded' ], true ) ) {
			...
			$email_args = [
				...
				'tokens'    => [
					...
					'order_amount' => hivepress()->woocommerce->format_price( $this->get_order_total( $order ) ),
	...
	public function add_listing_attributes( $attributes ) {
		...
		if ( get_option( 'hp_listing_allow_price_tiers' ) ) {
			...
			if ( ! hivepress()->get_version( 'bookings' ) ) {
				$attributes['price_tiers']['categories'] = $category_ids;
			}
		}
	...
	public function alter_order_totals( $rows, $order ) {
		...
		// Remove fees.
		$label = esc_html__( 'Service Fee', 'hivepress-marketplace' ) . ':';
		...
		$total  = $this->get_order_total( $order, false ) + $refund;
		...
		foreach ( $rows as $index => $row ) {
			if ( 'refund_0' === $index ) {
				$rows[ $index ]['value'] = wc_price( -$refund );
			} elseif ( strpos( $index, 'refund_' ) === 0 ) {
				unset( $rows[ $index ] );
			}
		}

		// Set total.
		$rows['order_total']['value'] = wc_price( $this->get_order_total( $order ) );
